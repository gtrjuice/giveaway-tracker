...
import { Copy, Download, Settings, Trash, Trophy, Package, TrendingUp, Gift, CheckCircle, Plus, Edit, RefreshCw } from "lucide-react"
...
const PACK_SIZES: { value: PackSize; label: string }[] = [
  { value: 10000,  label: "10K Pack" },
  { value: 20000,  label: "20K Pack" },
  { value: 45000,  label: "45K Pack" },
  { value: 100000, label: "100K Pack" },
]
...
const BLANK_FORM: OutcomeForm = {
  id: "", label: "", type: "immediate", entries: "1000000", group: "For You", customGroup: "",
}

const TYPE_OPTIONS: { value: WheelOutcomeType; label: string; desc: string }[] = [
  { value: "immediate",     label: "For You (immediate)",  desc: "Entries added directly to this order" },
  { value: "next_order",    label: "Next Order bonus",     desc: "Carried as a bonus to your next order" },
  { value: "multiplier_2x", label: "2× Order boost",       desc: "Doubles base pack entries (no value needed)" },
  { value: "fun",           label: "Fun / no entries",     desc: "Just for laughs — no entry value" },
]
...
export default function GiveawayTracker() {
  ...
  const spinOutcome  = useMemo(() => wheelOutcomes.find(w => w.id === spinId) ?? null, [spinId, wheelOutcomes])
  ...
  const preview = useMemo(() => {
    ...
    return { base, effBase, is2x, imm, next, total: effBase + extraEntries + pendingBonus + imm }
  }, [packSize, spinPending, spinOutcome, multipliers, pendingBonus, extraEntries])

  // resolved group for the form (custom or selected)
  ...
  const handleAddOrder = useCallback(() => {
    ...
    const is2x    = outcome?.type === "multiplier_2x"
    const effBase = is2x ? base * 2 : base
    const total   = effBase + extraEntries + pendingBonus + imm
    ...
    setOrders(prev => [{
      ...
      spinResult: outcome, immediateBonus: imm, nextOrderBonusGenerated: next,
      totalEntries: total, timestamp: new Date().toISOString(),
    }, ...prev])

    setPendingBonus(next)
    setOrderNum(""); setPackSize(""); setExtraInput(""); setSpinId(""); setSpinPending(false)
  }, [orderNum, packSize, extraEntries, spinPending, spinOutcome, multipliers, pendingBonus])

  const deleteOrder = useCallback((id: string) => setOrders(p => p.filter(o => o.id !== id)), [])

  const exportCSV = useCallback(() => {
    const hdr = ["Order #","Pack","Multiplier","Base","Extra/Promo","Bonus Applied","Spin Result","Spin Bonus","→ Next Bonus","Total","Time"]
    const rows = orders.map(o => [
      o.orderNumber, o.packSize ? `${o.packSize/1000}K` : "Custom",
      o.multiplier ? `${o.multiplier}x` : "—",
      o.baseEntries, o.extraEntries ?? 0, o.pendingBonusApplied,
      o.spinResult?.label ?? "Pending", o.immediateBonus, o.nextOrderBonusGenerated,
      o.totalEntries, new Date(o.timestamp).toLocaleString(),
    ])
  ...
  const clearAll = useCallback(() => {
    if (confirm("Clear ALL orders and reset pending bonus? Cannot be undone.")) {
      setOrders([]); setPendingBonus(0)
    }
  }, [])
  ...
  const openEdit = useCallback((w: WheelOutcome) => {
    ...
    setOForm({
      id: w.id, label: w.label, type: w.type,
      entries: String(w.entries), group: known ? w.group : "__custom__",
      customGroup: known ? "" : w.group,
    })
  ...
  const saveOutcome = useCallback(() => {
    ...
    const saved: WheelOutcome = {
      id: oForm.id || crypto.randomUUID(),
      label: oForm.label.trim(), type: oForm.type,
      entries: needsEntries ? Math.max(0, parseInt(oForm.entries) || 0) : 0,
      group: resolvedGroup || "For You",
    }
    setWheelOutcomes(prev =>
      outcomeMode === "add" ? [...prev, saved] : prev.map(w => w.id === saved.id ? saved : w)
    )
  ...
  const deleteOutcome = useCallback((id: string) => {
    ...
    if (spinId === id) { setSpinId(""); setSpinPending(false) }
  }, [spinId])

  const resetWheel = useCallback(() => {
    if (confirm("Reset all wheel outcomes back to the defaults?")) setWheelOutcomes(DEFAULT_WHEEL_OUTCOMES)
  }, [])
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="font-bold text-base leading-none">Giveaway Entry Tracker</h1>
            ...
            <Dialog open={settingsOpen} onOpenChange={o => { setSettingsOpen(o); if (o) setTempMults(multipliers) }}>
              ...
              <DialogContent className="bg-gray-900 border-gray-800 text-white max-w-sm">
                ...
                <div className="flex gap-2 pt-1">
                  <Button onClick={() => { setMultipliers(tempMults); setSettingsOpen(false) }}
                    className="flex-1 bg-emerald-600 hover:bg-emerald-700 h-9">Save</Button>
                  <Button variant="outline" className="border-gray-700 bg-gray-800 hover:bg-gray-700 h-9"
                    onClick={() => setTempMults(DEFAULT_MULTIPLIERS)}>Reset</Button>
                </div>
              </DialogContent>
            </Dialog>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { icon: <Package className="h-4 w-4 text-blue-400"/>,    label: "Orders Logged",     value: String(orders.length), sub: `${orders.filter(o => o.spinResult).length} with spin`, color: "text-blue-400" },
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              <Card className="bg-gray-900 border-gray-800">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-semibold flex items-center gap-2"><Plus className="h-4 w-4" /> New Order</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="icon" variant="outline" disabled={!orderNum}
                        ...
                        title="Copy for stream chat" onClick={() => copyText(orderNum, "form")}>
                        {copied === "form" ? <CheckCircle className="h-4 w-4 text-emerald-400" /> : <Copy className="h-4 w-4" />}
                      </Button>
                    </div>
                    {orderNum && <p className="text-xs text-gray-500">💬 Copy this to paste in the live stream chat to trigger your spin</p>}
                  </div>

                  {/* ② Entry pack */}
                  <div className="space-y-1.5">
                    <Label className="text-xs text-gray-400 uppercase tracking-wide">② Entry Pack <span className="text-gray-600 normal-case">(optional if using custom below)</span></Label>
                  ...
                  {/* ③ Additional / promo entries */}
                  <div className="space-y-1.5">
                    <Label className="text-xs text-gray-400 uppercase tracking-wide">③ Extra Entries <span className="text-gray-600 normal-case">(promo codes, bonuses, custom)</span></Label>
                    <Input
                    ...
                    />
                    {extraEntries > 0 && (
                      <p className="text-xs text-sky-400">+{fmt(extraEntries)} extra entries will be added to this order</p>
                    )}
                  </div>

                  {/* ④ Wheel spin */}
                  <div className="space-y-1.5">
                    <Label className="text-xs text-gray-400 uppercase tracking-wide">④ Wheel Spin Result</Label>
                    <Select value={spinPending ? "pending" : spinId}
                      onValueChange={v => { if (v === "pending") { setSpinPending(true); setSpinId("") } else { setSpinPending(false); setSpinId(v) } }}>
                      <SelectTrigger className="bg-gray-800 border-gray-700 text-white">
                        <SelectValue placeholder="Select spin result…" />
                      </SelectTrigger>
                      <SelectContent className="bg-gray-800 border-gray-700 max-h-72">
                        <SelectItem value="pending" className="text-amber-400 focus:bg-gray-700 focus:text-amber-400">⏳ Spin not done yet</SelectItem>
                        {Object.entries(wheelGroups).map(([group, outcomes]) => (
                          <div key={group}>
                            <div className="px-2 py-1 text-xs text-gray-500 font-semibold uppercase tracking-wide border-t border-gray-700 mt-1">{group}</div>
                            {outcomes.map(w => (
                              <SelectItem key={w.id} value={w.id} className="text-white focus:bg-gray-700 focus:text-white">
                                {w.label}
                                {w.entries > 0 && <span className="text-gray-400 ml-2">+{fmt(w.entries)}</span>}
                              </SelectItem>
                            ))}
                          </div>
                        ))}
                    ...
                    {(packSize || extraEntries > 0) && (
                      <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        <div className="space-y-1 text-sm">
                          ...
                          {preview.next > 0 && (
                            <div className="flex justify-between">
                              <span className="text-gray-400">→ Carries to next order</span>
                  ...
                  <Button onClick={handleAddOrder}
                    disabled={!orderNum.trim() || (!packSize && extraEntries <= 0)}
              ...
              <Card className="bg-gray-900 border-gray-800">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-semibold">📦 Current Pack Values</CardTitle>
                </CardHeader>
                ...
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" className="border-gray-700 bg-gray-800 hover:bg-gray-700 h-7 text-xs"
                      onClick={exportCSV} disabled={!orders.length}>
                    ...
                    <Button size="sm" variant="outline" className="border-red-900 bg-red-950/30 hover:bg-red-900/30 text-red-400 h-7 text-xs"
                      onClick={clearAll} disabled={!orders.length}>
              ...
              <CardContent className="p-0">
                {orders.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-gray-600 px-4">
                    ...
                    <p className="text-sm">No orders yet — go to Log Order to add your first</p>
                  </div>
                ) : (
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="border-gray-800 hover:bg-transparent">
                          ...
                          <TableHead className="text-gray-500 text-xs hidden md:table-cell">Extra</TableHead>
                          <TableHead className="text-gray-500 text-xs hidden md:table-cell">Bonus</TableHead>
                          <TableHead className="text-gray-500 text-xs">Spin</TableHead>
                          ...
                          <TableRow key={order.id} className="border-gray-800 hover:bg-gray-800/30">
                            <TableCell className="py-2.5">
                              <button onClick={() => copyText(order.orderNumber, order.id)}
                                className="flex items-center gap-1 font-mono text-xs text-blue-400 hover:text-blue-300 transition-colors" title="Click to copy">
                            ...
                            <TableCell className="py-2.5 text-xs hidden md:table-cell">
                              {(order.extraEntries ?? 0) > 0
                                ? <span className="text-sky-400">+{fmt(order.extraEntries)}</span>
                                : <span className="text-gray-700">—</span>}
                            </TableCell>
                            <TableCell className="py-2.5 text-xs hidden md:table-cell">
                              {order.pendingBonusApplied > 0
                                ? <span className="text-amber-400">+{fmt(order.pendingBonusApplied)}</span>
                                : <span className="text-gray-700">—</span>}
                            </TableCell>
                            <TableCell className="py-2.5">
                              {order.spinResult ? (
                                <div>
                                  <p className="text-xs text-gray-200 leading-tight">{order.spinResult.label}</p>
                                  <Badge variant="outline" className={cn("text-xs py-0 mt-0.5", badgeCls(order.spinResult.type))}>
                                    {order.immediateBonus > 0           ? `+${fmt(order.immediateBonus)}`
                                      : order.nextOrderBonusGenerated > 0 ? `→ +${fmt(order.nextOrderBonusGenerated)}`
                                      : order.spinResult.type === "multiplier_2x" ? "2× base"
                                      : "Fun 🎉"}
                                  </Badge>
                                </div>
                              ) : (
          ...
          <TabsContent value="wheel" className="mt-0">
            <Card className="bg-gray-900 border-gray-800">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <div>
                    <CardTitle className="text-sm font-semibold">🎡 Wheel Outcomes</CardTitle>
                    <p className="text-xs text-gray-500 mt-0.5">Add, edit, or remove outcomes. Changes apply immediately to the Log Order form.</p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="outline" onClick={resetWheel}
                      className="border-gray-700 bg-gray-800 hover:bg-gray-700 h-7 text-xs">
                            ...
                            <div key={w.id} className="flex items-center justify-between gap-2 rounded-lg bg-gray-800/50 border border-gray-700/60 px-3 py-2">
                              <div className="flex items-center gap-2 min-w-0">
                                <Badge variant="outline" className={cn("text-xs shrink-0", badgeCls(w.type))}>
                                  {w.entries > 0 ? `+${fmt(w.entries)}`
                                    : w.type === "multiplier_2x" ? "2× base"
                                    : "Fun"}
                                </Badge>
                              ...
                              <div className="flex gap-1 shrink-0">
                                <button onClick={() => openEdit(w)}
                                  className="p-1 rounded text-gray-500 hover:text-blue-400 hover:bg-blue-400/10 transition-colors" title="Edit">
                                  <Edit className="h-3.5 w-3.5" />
                                </button>
                                <button onClick={() => deleteOutcome(w.id)}
                                  className="p-1 rounded text-gray-500 hover:text-red-400 hover:bg-red-400/10 transition-colors" title="Delete">
                                  <Trash className="h-3.5 w-3.5" />
                                </button>
                              </div>
                            </div>
      ...
      <Dialog open={outcomeOpen} onOpenChange={setOutcomeOpen}>
        <DialogContent className="bg-gray-900 border-gray-800 text-white max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-white">{outcomeMode === "add" ? "Add Wheel Outcome" : "Edit Wheel Outcome"}</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 py-1">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={oForm.label} onChange={e => setOForm(p => ({ ...p, label: e.target.value }))}
                placeholder="e.g. 500K Entries, Next Order +500K…"
                className="bg-gray-800 border-gray-700 text-white placeholder:text-gray-600" />
            </div>
            ...
            {/* Entry value — only shown for immediate / next_order */}
            <AnimatePresence>
              {needsEntries && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                  className="space-y-1.5 overflow-hidden">
                  <Label className="text-sm text-gray-300">Entry Value</Label>
                  <Input type="number" min={0} value={oForm.entries}
                    onChange={e => setOForm(p => ({ ...p, entries: e.target.value }))}
                    placeholder="e.g. 1000000"
                    className="bg-gray-800 border-gray-700 text-white placeholder:text-gray-600" />
                  {parseInt(oForm.entries) > 0 && (
                    <p className="text-xs text-gray-400">= {fmt(parseInt(oForm.entries))} entries</p>
                  )}
                </motion.div>
              )}
            </AnimatePresence>
              ...
              <Select value={oForm.group} onValueChange={v => setOForm(p => ({ ...p, group: v, customGroup: "" }))}>
                ...
                <SelectContent className="bg-gray-800 border-gray-700">
                  {existingGroups.map(g => (
                    <SelectItem key={g} value={g} className="text-white focus:bg-gray-700 focus:text-white">{g}</SelectItem>
                  ))}
                ...
                {oForm.group === "__custom__" && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                    <Input value={oForm.customGroup} onChange={e => setOForm(p => ({ ...p, customGroup: e.target.value }))}
                      ...
                      className="bg-gray-800 border-gray-700 text-white placeholder:text-gray-600 mt-1.5" />
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
          ...
          <div className="flex gap-2 pt-1">
            <Button onClick={saveOutcome} disabled={!canSave}
              className="flex-1 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 h-9">
              {outcomeMode === "add" ? "Add Outcome" : "Save Changes"}
            </Button>
            <Button variant="outline" onClick={() => setOutcomeOpen(false)}
              className="border-gray-700 bg-gray-800 hover:bg-gray-700 h-9">Cancel</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
